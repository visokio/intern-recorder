(typeof define === 'function' && define.amd ? define : /* istanbul ignore next */ function (factory) {
	...
	RecorderProxy.prototype = {
		...
		_initializeOptions: function () {
			...
			input = document.getElementById('option-xpathBuilder');

			/* istanbul ignore if: the recorder is broken if this ever happens */
			if (!input) {
				throw new Error('Panel is missing input for option "xpathBuilder"');
			}

			input.onchange = function (event) {
				self.send('setXPathBuilder', [ event.target.value ]);
			};
		...
		setXPathBuilder: function (value) {
			if (this.contentWindow) {
				this.contentWindow.document.getElementById('option-xpathBuilder').value = value;
			}
		},
