(typeof define === 'function' && define.amd ? define : /* istanbul ignore next */ function (factory) {
	...
	function Recorder(chrome, storage) {
		...
		this.xpathBuilder = storage.getItem('intern.xpathBuilder');
	...
	Recorder.prototype = {
		...
		_initializePort: function () {
			...
			this.chrome.runtime.onConnect.addListener(function (port) {
				...
				/* istanbul ignore else: there are only two port types */
					...
					self._contentPort.send('setXPathBuilder', [ self.xpathBuilder ]);
				}
			});
		...
		_injectContentScript: function () {
			...
			this.chrome.tabs.executeScript(this.tabId, { file: 'lib/customxpath.js', allFrames: true });
		},
		...
		recordEvent: function (event) {
			...
			switch (event.type) {
				case 'click':
					...
					// Force a new "find" command:
					this._lastTarget = "(UNKNOWN)";
					// This allows cases like in jquery UI where clicking an element covers it with a modality layer
					// which means the new xpath gets erased by the above methods
					// and the next click gets associated with the wrong find. We force a new find for the next interaction.
		...
		refreshUi: function () {
			...
			this._port.send('setXPathBuilder', [ this.xpathBuilder ]);
		...
		setXPathBuilder: function (value) {
			this.storage.setItem('intern.xpathBuilder', value);
			this.xpathBuilder = value;
			this._contentPort.send('setXPathBuilder', [ value ]);
		},
